    ...
    public class BiliBiliLiveRequest : IBiliBiliLiveRequest
    {
        ...
        public async Task<bool> Connect(int roomID)
        {
            ...
            catch (Exception e)
            {
                ...
                chatPath = defaultPaths[random1.Next(0, defaultPaths.Length)];
                Debug.WriteLine($"获取弹幕服务器地址时出现错误，尝试使用默认服务器... 错误信息: {e}");
            }
            // 创建 TCP对象
            ...
            await client.ConnectAsync(ipAddress[random.Next(0, ipAddress.Length)], chatPort);
            netStream = Stream.Synchronized(client.GetStream());

            Debug.WriteLine("发送验证消息");
        ...
        async Task ReceiveMessageLoop()
        {
            ...
            while (this.connected)
            {
                try
                {
                    ...
                    {
                        Debug.WriteLine("协议失败: (L:" + protocol.PacketLength + ")");
                        continue;
                    }
